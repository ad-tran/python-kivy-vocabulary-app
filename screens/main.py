...
class VocabularyApp(DictionaryScreen, ExpressionsScreen, LearnScreen, ReviewScreen, DashboardScreen, BoxLayout):
    ...
    def _build_ui(self):
        ...
        self.word_label = Label(
            text="Click ‘Next word’ to begin. All available words are B1 level (Cambridge)",
            font_size=sp(54),
            size_hint=(1, 0.4),
        ...
        # Enable wrapping when width gets smaller
        def _sync_word_label(*_):
            try:
                self.word_label.text_size = (self.word_label.width - 12, None)
            except Exception:
                pass
        self.word_label.bind(size=lambda *_: _sync_word_label())
        _sync_word_label()

        self.progress_label = Label(text="0 words remaining", font_size=sp(22), size_hint=(1, 0.1), color=(0.8, 0.8, 0.8, 1))
        self.add_widget(self.progress_label)

        # Hinweis unter dem Zähler
        self.hint_label = Label(
            text="Note: The current word is automatically marked as 'Known' when you click 'Next word'. "
                 "Tap 'New word' to move it to 'New words'. "
                 "Double‑tap a removed word to restore it.",
            font_size=sp(12),
            size_hint=(1, 0.08),
            color=(0.8, 0.8, 0.8, 1),
        ...
        def _sync_hint(*_):
            try:
                self.hint_label.text_size = (self.hint_label.width - 12, None)
            except Exception:
                pass
        self.hint_label.bind(size=lambda *_: _sync_hint())
        _sync_hint()
        ...
        self.known_button = Button(text="Correct", font_size=30, background_color=(0.2, 0.6, 0.9, 1))
        ...
        self.known_header_btn = Button(text="Known words (0/0)", size_hint=(1, 0.12), font_size=24, background_normal='', background_color=(0.0, 0.35, 0.0, 1), color=(0.95, 0.98, 1, 1))
        ...
        self.removed_header_btn = Button(text="Removed words (0)", size_hint=(1, 0.12), font_size=20, background_normal='', background_color=(0.25, 0.0, 0.0, 1), color=(0.95, 0.98, 1, 1))
        ...
        def _style_top(btn):
            ...
            try:
                ...
                btn.valign = 'middle'
                btn.padding = (8, 4)
                btn.shorten = False  # nicht kürzen, sondern umbrechen
                # Wrap aktivieren: Textbreite an Button-Breite koppeln
                def _sync(inst, *_):
                    inst.text_size = (inst.width - 12, None)
                btn.bind(size=_sync)
                _sync(btn)
            except Exception:
                pass

        # WIRKLICHE Attributnamen der 7 Header-Buttons
        for name in (
            "add_words_button",
            "words_button",          # <- dieser ist "Neue Wörter von Text überprüfen"
            "expressions_button",
            "learn_button",
            "learned_main_btn",
            "review_main_btn",
    ...
    def next_word(self, instance):
        ...
        if new_word is None:
            ...
            self.progress_label.text = f"{self.remaining_count} words remaining"
            return
    ...
    def update_display(self):
        ...
        self.progress_label.text = f"{self.remaining_count} Words left"
        # Hinweis immer anzeigen
        if getattr(self, "hint_label", None):
    ...
    def update_lists(self):
        ...
        self.removed_header_btn.text = f"Removed words ({len(removed_display)})"
        ...
        self.known_header_btn.text = f"Known words ({known_count}/{total})"
        self.new_header_btn.text = f"New words ({new_count}/{total})"
    ...
    def open_known_list_popup(self, *_):
        ...
        self._open_list_popup("Known words", words, show_tt_filter=True, initial_only_tw=False)
    ...
    def open_new_list_popup(self, *_):
        ...
        words = list(reversed(words))
        words = [w for w in words if w.lower() not in self.removed_words]
        self._open_list_popup("New words", words)
    ...
    def _open_list_popup(self, title, words, *, show_tt_filter: bool = False, initial_only_tw: bool = False):
        ...
        search_input = TextInput(hint_text="Search...", multiline=False, font_size=20, size_hint=(1, 1))
        ...
        def rebuild_list(query_text: str):
            ...
            if len(filtered) > max_items:
                grid.add_widget(Label(text=f"… {len(filtered)-max_items} more …", size_hint_y=None, height=32, font_size=18, color=(0.8,0.8,0.8,1)))
        rebuild_list("")
        _pending = {"ev": None}
        def _debounced(*_):
            if _pending["ev"]:
        ...
        close_btn = Button(text="Close", font_size=18, size_hint=(1, 1), background_color=self.theme["closeButton"])
    ...
    def open_add_words_popup(self, *_):
        ...
        self.add_popup = Popup(title="Add new words", content=content, size_hint=(0.9, 0.9), auto_dismiss=True)
    ...
    def _commit_added_words(self, *_):
        ...
        self._store.save_async()
        if added:
            self.show_error_popup(f"{added} new words added.", duration=2)
    ...
    def open_text_check_popup(self, *_):
        content = BoxLayout(orientation='vertical', spacing=10, padding=12)
        info = Label(
            text="Paste plain text. Words found will be checked\nagainst your vocabulary (case-insensitive, length > 2).",
    ...
    def _analyze_text_and_show_results(self, *_):
        ...
        root = BoxLayout(orientation='vertical', spacing=10, padding=12)
        header = Label(
            text=f"In text: {len(uniques)} unique • New: {len(unknown)} • Known: {len(known_in)}",
        ...
        if unknown:
            ...
            gl.add_widget(Label(text="No new words found.", size_hint_y=None, height=48, font_size=24, color=(0.6,0.9,0.7,1)))
        sv.add_widget(gl)
        root.add_widget(sv)
        bar = BoxLayout(size_hint=(1, 0.16), spacing=10)
        # Fix: Close-Button mit fester Optik
        close_btn = Button(
            text="Close",
            font_size=20,
            background_normal='',
            background_color=self.theme["closeButton"],
            color=self.theme["text"]
        )
        add_btn = Button(text=f"Add ({len(unknown)})", font_size=20, background_color=self.theme["closeButton"])
    ...
    def _extract_words_from_free_text(self, text: str):
        # erlaubt z. B. far-out, it's, mother-in-law
        tokens = re.findall(r"[A-Za-z]+(?:[-'][A-Za-z]+)*", text or "")
        words = [t.lower() for t in tokens if len(t) > 2]
        # unique + sortiert für stabile Anzeige
    ...
    def _commit_add_from_text_check(self, *_):
        ...
        self.show_error_popup(f"{len(to_add)} words added to the vocabulary. Use 'Next word' to classify them.", duration=3)
    ...
    def open_all_learned_popup(self, *_, initial_only_tw: bool = False):
        if not self.learned_session and not self.expressions:
            self.show_error_popup("No learned words or expressions.")
            return
        root = BoxLayout(orientation='vertical', spacing=10, padding=12)
        ...
        bar.add_widget(close_btn)
        root.add_widget(bar)
        self.learned_popup = Popup(title="Learned words", content=root, size_hint=(0.95, 0.92), auto_dismiss=True)
        ...
        def rebuild_list(query_text: str):
            ...
            header.text = f"{total} learned words & expressions" + (" • TT" if only_tw else "")

            if not filtered:
                gl.add_widget(Label(text="No results.", size_hint_y=None, height=40, font_size=20, color=(0.8,0.8,0.8,1)))
                return
            ...
            gl.add_widget(Label(text="Loading …", size_hint_y=None, height=32, font_size=18, color=(0.8,0.8,0.8,1)))
            ...
            def add_chunk(dt=None):
                ...
                for w in items[start:end]:
                    ...
                    if not entries:
                        self._add_wrapped_label(gl, "– No meanings available –", 20, color=(0.8, 0.8, 0.8, 1), indent_left=12)
                    else:
                ...
                if end < len(items):
                    ...
                    if overflow > 0:
                        gl.add_widget(Label(text=f"… {overflow} more …", size_hint_y=None, height=32, font_size=18, color=(0.8,0.8,0.8,1)))
    ...
    def open_correct_word_popup(self, *_):
        if not self.current_word:
            ...
            return
        root = BoxLayout(orientation='vertical', spacing=10, padding=12)
        info = Label(text="Correct word and save:", font_size=18, size_hint=(1, 0.12), color=(0.9, 0.95, 1, 1))
        ...
        save_btn = Button(text="Save", font_size=20, background_color=(0.2, 0.6, 0.2, 1))
        bar.add_widget(cancel_btn); bar.add_widget(save_btn)
        root.add_widget(bar)
        self.correct_popup = Popup(title="Correct word", content=root, size_hint=(0.9, 0.5), auto_dismiss=True)
    ...
    def _init_stt_async(self):
        def _on_loaded(stt_obj):
            self.stt = stt_obj
            # im UI-Thread Autostart auslösen
            Clock.schedule_once(lambda dt: self._notify_stt_ready(), 0)
        # rufe _on_loaded, wenn dein Lade-Thread fertig ist
